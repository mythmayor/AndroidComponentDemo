...
public class PinyinUtil {
    ...
    private static int getAscii(String chs) {
        ...
        try {
            ...
            Log.i(TAG, "PinyinUtil - getAscii: " + "ERROR:ChineseSpelling.class-char2Ascii(String chs)" + e);
        }
        return asc;
    }
     ...
     * @return 中国->"zg"
     */
    public static String getPinYinInitials(String source) {
        if (source == null || source.isEmpty()) {
            return null;
        }
        StringBuilder pinyinList = new StringBuilder();
        for (int i = 0; i < source.length(); i++) {
            ...
            if (item.getBytes().length >= 2) {
                ...
                if (pinyin.length() > 0) {
                    pinyinList.append(pinyin.substring(0, 1));
                }
            } else {
                if (item.length() > 0) {
                    pinyinList.append(item.substring(0, 1));
                }
            }
        }
        return pinyinList.toString();
